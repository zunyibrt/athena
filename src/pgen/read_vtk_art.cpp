...
//factor to translate radiation field to Draine units
static const Real fchi=0.26;
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  if (isjoinedvtk != 0) {
    ...
    for(int i = 0; i < scaler_fields.size(); ++i) {
      ...
      } else if (scaler_fields[i] == "CR_ionization_rate") {
        if (Globals::my_rank == 0) {
          readvtk(this, vtkfile, "CR_ionization_rate", 0, data, isjoinedvtk);
        }
#ifdef MPI_PARALLEL
        ierr = MPI_Bcast(data.data(), Nx_mesh*Ny_mesh*Nz_mesh,
            MPI_ATHENA_REAL, 0, MPI_COMM_WORLD);
          ...
          for (int j=js; j<=je; ++j) {
            for (int i=is; i<=ie; ++i) {
              for (int iang=0; iang < prad->nang; ++iang) {
                prad->ir(k, j, i,
                    pspec->pchemnet->index_cr_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis);
              }
            }
          }
        }
      } else if (scaler_fields[i] == "rad_energy_density_PE") {
        if (Globals::my_rank == 0) {
          readvtk(this, vtkfile, "rad_energy_density_PE", 0, data, isjoinedvtk);
        }
#ifdef MPI_PARALLEL
        ierr = MPI_Bcast(data.data(), Nx_mesh*Ny_mesh*Nz_mesh,
            MPI_ATHENA_REAL, 0, MPI_COMM_WORLD);
        ...
        for (int k=ks; k<=ke; ++k) {
          for (int j=js; j<=je; ++j) {
            for (int i=is; i<=ie; ++i) {
              for (int iang=0; iang < prad->nang; ++iang) {
                prad->ir(k, j, i,
                    pspec->pchemnet->index_gpe_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
              }
            }
          }
        }
      } else if (scaler_fields[i] == "rad_energy_density_LW") {
        if (Globals::my_rank == 0) {
          readvtk(this, vtkfile, "rad_energy_density_LW", 0, data, isjoinedvtk);
        }
#ifdef MPI_PARALLEL
        ierr = MPI_Bcast(data.data(), Nx_mesh*Ny_mesh*Nz_mesh,
            MPI_ATHENA_REAL, 0, MPI_COMM_WORLD);
        ...
        for (int k=ks; k<=ke; ++k) {
          for (int j=js; j<=je; ++j) {
            for (int i=is; i<=ie; ++i) {
              for (int iang=0; iang < prad->nang; ++iang) {
                prad->ir(k, j, i,
                    pspec->pchemnet->iph_H2_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
              }
            }
          }
        }
      } else if (scaler_fields[i] == "rad_energy_density2") {
        if (Globals::my_rank == 0) {
          readvtk(this, vtkfile, "rad_energy_density2", 0, data, isjoinedvtk);
        }
#ifdef MPI_PARALLEL
        ierr = MPI_Bcast(data.data(), Nx_mesh*Ny_mesh*Nz_mesh,
            MPI_ATHENA_REAL, 0, MPI_COMM_WORLD);
        ...
        for (int k=ks; k<=ke; ++k) {
          for (int j=js; j<=je; ++j) {
            for (int i=is; i<=ie; ++i) {
              for (int iang=0; iang < prad->nang; ++iang) {
                prad->ir(k, j, i,
                    pspec->pchemnet->iph_CHx_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
              }
            }
          }
        }
      } else if (scaler_fields[i] == "rad_energy_density3") {
        if (Globals::my_rank == 0) {
          readvtk(this, vtkfile, "rad_energy_density3", 0, data, isjoinedvtk);
        }
#ifdef MPI_PARALLEL
        ierr = MPI_Bcast(data.data(), Nx_mesh*Ny_mesh*Nz_mesh,
            MPI_ATHENA_REAL, 0, MPI_COMM_WORLD);
        ...
        for (int k=ks; k<=ke; ++k) {
          for (int j=js; j<=je; ++j) {
            for (int i=is; i<=ie; ++i) {
              for (int iang=0; iang < prad->nang; ++iang) {
                prad->ir(k, j, i,
                    pspec->pchemnet->iph_OHx_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
                prad->ir(k, j, i,
                    pspec->pchemnet->iph_Si_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
              }
            }
          }
        }
      } else if (scaler_fields[i] == "rad_energy_density4") {
        if (Globals::my_rank == 0) {
          readvtk(this, vtkfile, "rad_energy_density4", 0, data, isjoinedvtk);
        }
#ifdef MPI_PARALLEL
        ierr = MPI_Bcast(data.data(), Nx_mesh*Ny_mesh*Nz_mesh,
            MPI_ATHENA_REAL, 0, MPI_COMM_WORLD);
        ...
        for (int k=ks; k<=ke; ++k) {
          for (int j=js; j<=je; ++j) {
            for (int i=is; i<=ie; ++i) {
              for (int iang=0; iang < prad->nang; ++iang) {
                prad->ir(k, j, i,
                    pspec->pchemnet->iph_CO_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
                prad->ir(k, j, i,
                    pspec->pchemnet->iph_C_ * prad->nang + iang)
                  = data(k-ks+gks, j-js+gjs, i-is+gis) * fchi;
              }
            }
          }
